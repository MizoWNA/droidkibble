...
| Pressing the power button doesn't light the screen | Check `tail /data/adb/phoneserver/phoneserverd.log`. "power button: /dev/input/eventN" at startup means the key was found; "power button pressed" should appear when you press it. "no display files" means `status.jar` and `run.sh` aren't in `/data/adb/phoneserver/display/` (run `install.sh` after building it). The daemon must be 1.0.0 or newer (`phoneserverd --version`). |
| The screen lights up but shows nothing | Read `/data/adb/phoneserver/display.log`. If it says `surfaceflinger is not running`, something stopped SurfaceFlinger, which the display needs. |
